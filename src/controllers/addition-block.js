...
export default class AdditionBlockController {
  constructor(container, moviesModel, onDataChange, commentsModel, api) {
  ...
  render() {
    ...
    if (oldFilmsSortByCommentsCount) {
      ...
      this._showingFilms = [].concat(this._showingFilms, renderFilms(mostCommentedBlockElement, this._getFilmsSortByCommentsCount(), this._onDataChange, this._filmCommentsModel, this._api));
    } else {
      for (let i = 0; i < FILM_COUNT_ADDITION; i++) {
        ...
        if (ADDITION_CONTAINER_TITLES[i] === `Top rated` && films[0].rating > 0) {
          ...
          this._showingFilms = this._showingFilms.concat(renderFilms(additionContainerElementFilmList, films, this._onDataChange, this._filmCommentsModel, this._api));
